...
def get_embeddings_and_labels(file_name, data_path, device, layer_num):
    ...
    """
    labels = torch.load('dataset/processed_multitude/labels/' + file_name + '.pt').to(device)
    try:
        embeddings = torch.load(f'save/multitude_embeddings/' + file_name + '.pt')
    ...
    if layer_num != -1:
        ...
        with open(f'save/multitude_kl_divergence/all_tokens/' + file_name + '.pkl', 'rb') as f:
...
def get_train_eval_data(layer_num, device, language, train_num=150, valid_num=20):
    """
    Get train and validation data from dataset of a specific language.
    """
    train_name = language + '_train'
    val_name = language + '_val'
    train_path = 'dataset/processed_multitude/train_valid_data/' + language + '_train.json'
    val_path = 'dataset/processed_multitude/train_valid_data/' + language + '_val.json'
...
def get_test_data(device, language, layer_num) :
    ...
    for model_name in ['gpt-3.5-turbo', 'gpt-4']:
        test_embeddings, test_labels = get_embeddings_and_labels(
            f'{language}_{model_name}', 
            f'dataset/processed_multitude/test_data/{language}_{model_name}.json', device, layer_num)
        testset_embeddings.append(test_embeddings)
        testset_labels.append(test_labels)
        data_and_model_names.append(language + "-" + model_name)
...
def train(layer_num, device, droprate, num_epochs, learning_rate, language, hidden_sizes=[1024, 512]):
    """
    Train a binary classifier on embeddings.
    """
    train_embeddings, train_labels, valid_embeddings, valid_labels = get_train_eval_data(layer_num, device, language)
...
def group_and_average(name_to_auroc):
    ...
    for name, auroc in name_to_auroc.items():
        ...
        if 'gpt-3.5-turbo' in name_lower:
    ...
    avg_aurocs = {}
    for model_type in ['gpt-3.5-turbo', 'gpt-4']:
...
def run_all_tests(model, layer_num, device, language) :
    """
    Runs the trained model on all test datasets (across models like GPT-3.5, GPT-4, Claude) and computes AUROC.
    """
    testset_embeddings, testset_labels, data_and_model_names = get_test_data(device, language, layer_num)
    with torch.no_grad():
        name_to_auroc = {}
...
def train_and_test_one_layer(layer_num, device,
    hidden_sizes=[1024, 512], droprate = 0.4, num_epochs = 10, learning_rate = 0.0003, language = 'cs'):
    """
    ...
    """
    model = train(layer_num, device, droprate, num_epochs, learning_rate, language, hidden_sizes)
    name_to_auroc = run_all_tests(model, layer_num, device, language)
...
if __name__ == '__main__':
    ...
    if (args.layer_num >= -1) :
        avg_aurocs = train_and_test_one_layer(args.layer_num, device, [1024, 512], args.dropout, args.epochs, args.lr, args.language)
        print(avg_aurocs)
    
    else:
        layer_to_aurocs = {'gpt-3.5-turbo': [], 'gpt-4': []}
        layer_nums = []

        for layer_num in range(33):
            print("layer_num =", layer_num)
            layer_nums.append(layer_num)
            avg_aurocs = train_and_test_one_layer(layer_num, device, [1024, 512], args.dropout, args.epochs, args.lr, args.language)
            for model_type in ['gpt-3.5-turbo', 'gpt-4']:
        ...
        try:
            ...
            plt.savefig('figures/' + args.language + '.png')
            print('Plot saved as figures/' + args.language + '.png (GUI not available)')



# python multilingual.py --layer_num -1 --epochs 10 --dropout 0.4 --lr 0.0003 --language 'cs'
# python multilingual.py --layer_num -2 --epochs 10 --dropout 0.4 --lr 0.0003 --language 'cs'